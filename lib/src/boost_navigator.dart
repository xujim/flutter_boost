...
FlutterBoostRouteFactory routeFactoryWrapper(FlutterBoostRouteFactory routeFactory) {
  return (settings, uniqueId) {
    var route = routeFactory(settings, uniqueId);
    if (route == null && settings.name == '/') {
      route = PageRouteBuilder<dynamic>(settings: settings, pageBuilder: (_, __, ___) => Container());
    }
    return route;
  };
}
...
///
/// 一个包装类，底层基于appstate实现，方便外层调用进行push ，pop，主要供flutter层调用
class BoostNavigator {
  ...
  Future<T> push<T extends Object?>(String name, {Map<String, dynamic>? arguments, bool withContainer = false, bool opaque = true}) {
    assert(appState != null, 'Please check if the engine has been initialized!');
    ...
    if (isFlutter && withContainer) {
      ...
      return appState!.pushWithResult(name, arguments: arguments, withContainer: withContainer, opaque: opaque);
    } else {
      // 2. open native page or flutter page without container
      return appState!.pushWithInterceptor(name, false /* isFromHost */, isFlutter, arguments: arguments, withContainer: withContainer, opaque: opaque);
    }
  }
  ...
  Future<T> pushReplacement<T extends Object?>(String name, {Map<String, dynamic>? arguments, bool withContainer = false}) async {
    final String? id = getTopPageInfo()?.uniqueId;
    final result = push<T>(name, arguments: arguments, withContainer: withContainer);
  ...
  PageInfo? getPageInfoByContext(BuildContext context) => BoostContainer.of(context)?.pageInfo;
